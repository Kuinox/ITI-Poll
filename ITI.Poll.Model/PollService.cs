...
namespace ITI.Poll.Model
{
	public sealed class PollService : IPollService
	{
		...
		static readonly string AuthorAccountDeletedMsg = "The author account is deleted and cannot create a new poll.";
		public async Task<Result<Poll>> CreatePoll(IUnitOfWork unitOfWork, IPollRepository pollRepository, IUserRepository userRepository, NewPollDto newPollInfo)
		{
			...
			Result validation = Validate(newPollInfo, author.Value);
		...
		Result Validate(NewPollDto newPollInfo, User author)
		{
			if (author.IsDeleted)
				return Result.CreateError(Errors.AccountDeleted, AuthorAccountDeletedMsg);
			...
			if (newPollInfo.GuestNicknames.Select(n => n.ToLowerInvariant()).Contains(author.Nickname.ToLowerInvariant()))
